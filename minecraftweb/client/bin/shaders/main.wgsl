...
fn buildFaceVertex( face : u32, uv : vec2<u32>, packedMaterial : u32 ) -> vec4<u32>
{
  ...
  let topTex = packedMaterial & 0xFFu;
  let bottomTex = (packedMaterial >> 8u) & 0xFFu;
  let sideTex = (packedMaterial >> 16u) & 0xFFu;
...
{
  let viewDir = normalize(camera.CamLoc.xyz - input.fragPos);
  ...
  let finalColor = texColor.rgb + specular;

  return vec4<f32>(vec3f(finalColor), 1.0);
